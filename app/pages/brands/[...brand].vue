<template>
  <UContainer>
    ...
    >
      ...
      <template #links>
        <div class="flex flex-col gap-4">
          <div class="flex items-start gap-2" v-if="brand?.categories?.length > 0">
            <p>Categories:</p>
            <UBadge variant="soft" v-for="category in brand?.categories">
              {{ category }}
            </UBadge>
          </div>
          <div v-else>
          ...
          <div class="flex items-start gap-2" v-if="brand?.tags?.length > 0">
            <p>Tags:</p>
    ...
    <!-- CTA section to visit & share brand -->
    ...
    <div class="flex items-center justify-between my-4">
      <h2 class="text-4xl font-bold mb-0">Similar brand</h2>
      <UButton
        variant="subtle"
        :to="
          brand?.categories?.length
            ? `/brands?category=${encodeURIComponent(brand.categories[0])}`
            : '/brands'
        "
      >
        View All
    ...
    <div
      v-if="brands"
      class="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-5 my-8 gap-4"
    >
...
<script setup>
  ...
  const { data: brands } = await useAsyncData('brands-' + route.params.brand.at(-1), () => {
    return getItems({
      collection: 'brands',
      params: {
